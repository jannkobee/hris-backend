...
Ensure all operational primitives are configured and verified before promoting Suitify HR to public production.
...
- [ ] **Test Delivery**: Send test email and verify DKIM/SPF passing:
    ```bash
    php artisan tinker --execute="Mail::raw('Suitify HR staging ping', function(\$m) { \$m->to('admin@example.com')->subject('Delivery Test'); });"
    ```
...
- [ ] **Automated SQL Backup**:
    ...
    Verify backup file is written to `storage/app/backups/suitify_hr_backup_*.sql.gz` and recorded in `platform_operation_logs`.

- [ ] **Test Database Restoration**:
      Verify backup can be cleanly decompressed and loaded with foreign keys restored:
    ```bash
    php artisan db:restore suitify_hr_backup_YYYY_MM_DD_HHMMSS.sql.gz --force
    ```
...
- [ ] **Webhook Endpoint**: Register `https://your-domain.com/backend/api/v1/billing/stripe/webhook` in the Stripe Dashboard.
- [ ] **Event Subscriptions**:
