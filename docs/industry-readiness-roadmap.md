...
**Benefits & Expense Claims Enterprise Improvements (2026-09-11):** Delivered private receipt attachments (`organizations/{id}/expense-receipts`) with tenant-isolated, ownership-verified streaming preview and download (`GET /api/v1/expense-claims/{id}/receipt`). Implemented Philippine statutory de minimis benefit ceiling intelligence (`GET /api/v1/benefit-plans/de-minimis-ceilings`) complying with BIR RR 2-98 and RR 11-2018, complete with real-time excess taxable threshold alerts. Added full Benefit Plan updates (`PATCH /api/v1/benefit-plans/{id}`), enrolled employee roster modal (`GET /api/v1/benefit-plans/{id}/enrollments`), and unenrollment cancellation (`DELETE /api/v1/benefit-enrollments/{id}`). Implemented accounting CSV export (`GET /api/v1/expense-claims/export`) for finance processing. Full test suite: 199 tests passed (1,769 assertions); tenancy audit passed for 69 tables; strict authorization audit passed 100%; frontend build passed (656 modules, 12.01s).
...
**Container billing configuration (2026-09-10):** Both local and production Compose files now forward `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `BILLING_PORTAL_RETURN_HOSTS` to PHP services, including queue and scheduler. The deployment template and guide document staging credentials, return hosts, and service recreation after changes. Locally verified: 4 Docker Compose configuration regression tests passed with dummy values, checking service inheritance, blank-credential defaults, and exclusion from frontend settings/build arguments. No containers, migrations, or Stripe requests were executed. Staging runtime verification remains pending; next supply isolated credentials and exercise checkout, webhooks, portal, queue, and reconciliation. Annual pricing and Basic allowance grandfathering remain open decisions.

**Configurable Growth minimum (2026-09-10):** Owner selected no minimum. Platform Pricing now persists `minimum_billable_employees` (default 0) per version and exposes a numeric console control; 1 enables a one-employee-rate floor. Checkout and subscription metadata preserve the setting; billing previews use it. Zero-cost signup retains the agreed paid rate for subsequent reconciliation. Existing metadata without a minimum defaults to zero. Verified: 10 focused backend tests / 40 assertions and 5 pricing utility tests passed. Stripe test-mode verification of the zero-cost-to-paid transition is still required; annual policy and Basic grandfathering remain open.
...
Frontend production build passed for the 2026-09-10 pricing-display changes.
...
**Billing reliability delivery (2026-09-10):** Pricing editor, effective versions, history, public calculator and checkout quantity are implemented. Subscription synchronization uses Stripe subscription metadata for the agreed free allowance, preserves the subscribed price, skips unchanged quantities, and throws on failed provider requests or missing subscription items. Legacy subscriptions without allowance metadata are left untouched. Reconciliation reports per-organization failures and retries on the next daily run. Webhooks require an event ID, persist a global receipt atomically with subscription changes, ignore unpaid checkout completion, handle asynchronous payment success, and reject signatures outside a five-minute window. Signup displays the published free allowance. Focused billing verification: 8 tests, 31 assertions passed. No new migration; receipts use the existing global platform settings table.

**Next release verification:** Exercise checkout, asynchronous payments, duplicate webhooks, quantity changes, failed-payment recovery and reconciliation against Stripe test mode on staging. Automated provider requests are mocked; live provider verification remains outstanding.
...
**Product logo (2026-09-08):** Wired the original product logo into the marketing homepage, trial signup, customer login, Platform Console login/sidebar, and bundled PNG favicon. A shared logo component preserves aspect ratio; tenant-uploaded workspace logos and initials are unchanged. Prettier, two branding regression tests, six existing routing/pricing tests, and the production build pass. Smaller optimized icon derivatives remained a possible performance improvement at that point.

**Marketing homepage (2026-09-08):** Moved the public view to `src/views/Marketing/Home.vue` at `/`, with the product title and tagline. `/saas` and its existing route name redirect to Home while preserving query parameters and anchors. Public website links now use `/`. The authenticated shell entry is `/app` (redirects to Dashboard); existing module URLs and authentication requirements are preserved. Prettier, six combined routing/pricing tests, and the production build pass (build required an elevated retry for esbuild parent-directory access).
